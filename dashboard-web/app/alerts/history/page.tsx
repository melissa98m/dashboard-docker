...
export default function AlertsHistoryPage() {
  ...
  const [historyMetricFilter, setHistoryMetricFilter] = useState<MetricType | "">("");
  ...
  const [historySort, setHistorySort] = useState<"created_at_desc" | "created_at_asc">("created_at_desc");
  ...
  const buildHistoryUrl = (offset: number) => {
    ...
    if (historyContainerFilter) params.set("container_id", historyContainerFilter);
    if (historyMetricFilter) params.set("metric_type", historyMetricFilter);
    if (historyTriggeredByFilter !== "all") params.set("triggered_by", historyTriggeredByFilter);
    if (historySinceHours.trim()) params.set("since_hours", historySinceHours.trim());
  ...
  const loadHistory = async ({ reset }: { reset: boolean }) => {
    ...
    try {
      ...
      const payload = await apiJson<AlertHistoryResponse>(buildHistoryUrl(targetOffset));
  ...
  }, [historyContainerFilter, historyMetricFilter, historyTriggeredByFilter, historySinceHours, historySort]);

  const onRestartFromHistory = async (item: AlertHistoryItem) => {
    if (item.rule_id == null) return;
    setRestartingRuleId(item.rule_id);
    const label = item.container_name || item.container_id || `rule #${item.rule_id}`;
    try {
  ...
  return (
    <main className="page-shell p-4 max-w-4xl mx-auto space-y-4">
      <div className="page-header">
        <h1 className="page-title text-2xl font-bold">Historique des alertes</h1>
        <div className="top-nav">
          <Link href="/alerts/rules">Regles</Link>
      ...
      <section className="panel">
        ...
        <div className="grid gap-2 mb-3">
          <label>
            ...
            <select
              ...
              onChange={(event) => setHistoryContainerFilter(event.target.value)}
          ...
          <label>
            <span className="field-label">Filtre metrique</span>
            <select
              value={historyMetricFilter}
              onChange={(event) => setHistoryMetricFilter(event.target.value as MetricType | "")}
              className="w-full rounded bg-slate-900 px-3 py-2 border border-slate-700"
            >
          ...
          <label>
            ...
            <select
              ...
              onChange={(event) =>
                setHistoryTriggeredByFilter(event.target.value as "all" | "manual" | "alert-engine")
              }
              className="w-full rounded bg-slate-900 px-3 py-2 border border-slate-700"
            >
          ...
          <label>
            <span className="field-label">Fenetre en heures</span>
            <input
              value={historySinceHours}
              onChange={(event) => setHistorySinceHours(event.target.value)}
              placeholder="Fenetre heures (ex: 24)"
              type="number"
              min="1"
              max={24 * 30}
              className="w-full rounded bg-slate-900 px-3 py-2 border border-slate-700"
          ...
          <label>
            ...
            <select
              value={historySort}
              onChange={(event) =>
                setHistorySort(event.target.value as "created_at_desc" | "created_at_asc")
              }
              className="w-full rounded bg-slate-900 px-3 py-2 border border-slate-700"
            >
              <option value="created_at_desc">Plus recentes d&apos;abord</option>
              <option value="created_at_asc">Plus anciennes d&apos;abord</option>
            </select>
          </label>
        </div>
        ...
        {history.length === 0 && <p className="text-slate-400">Aucun declenchement recent.</p>}
        <ul className="space-y-2">
          {history.map((item) => (
            <li key={item.id} className="entity-card flex items-center justify-between gap-2">
              <div>
                <p className="font-medium">{item.container_name || item.container_id || "Container inconnu"}</p>
                <p className="text-xs text-slate-400">
                  {item.metric_type ? metricLabels[item.metric_type] : "metric"} · valeur{" "}
              ...
              <button
                ...
                disabled={!item.can_restart || item.rule_id == null || restartingRuleId === item.rule_id}
